...
def custom_transform(mat):
    ...
    # 根据随机重排的行索引创建新的矩阵
    for i in range(rows):
        new_row_index = row_indices[i]
        new_row = mat[new_row_index, :]
        transformed_mats.append(new_row)

    # 创建一个空的 numpy 数组，用于存储生成的多个新数据
    rolled_mats = []
    stacked_tensor = torch.stack(transformed_mats)
    
    # # 遍历矩阵的每一行
    # for i in range(rows):
    #
    #     # 调用 roll_matrix 函数对当前行进行循环移动
    #     rolled_mat = roll_matrix(stacked_tensor)
    #     # 将移动后的行添加到 rolled_mats 列表中
    #     rolled_mats.append(rolled_mat)
    #     stacked_tensor=rolled_mat
...
# # 定义输入路径和输出路径
# input_path = "D:/DFCdata/DFC/train"  # 输入路径
