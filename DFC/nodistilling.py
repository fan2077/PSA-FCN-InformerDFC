...
class EncoderLayer(nn.Module):
    def __init__(self, attention,  norm_shape, ffn_num_input, ffn_num_hiddens,
        ...
        self.addnorm1 = AddNorm(norm_shape, Encoder_dropout)
        self.ffn = PositionWiseFFN(
            ffn_num_input, ffn_num_hiddens,ffn_num_input)
        self.dropout = nn.Dropout(Encoder_dropout)
        self.addnorm2 = AddNorm(norm_shape, Encoder_dropout)
    ...
    def forward(self, x, attn_mask=None):
        ...
        Y = self.addnorm1(x,  self.dropout(new_x))
        # print("Y.shape",Y.shape)
        return self.addnorm2(Y, self.ffn(Y)),attn
...
class Informer(nn.Module):
    def __init__(self, c_out, factor, d_model, n_heads, e_layers,ffn_num_hiddens,norm_shape,ffn_num_input,
        ...
        # self.mlp = Mlp(in_features=d_model, hidden_features=d_model*2)
        # self.convlayer = ConvLayer(d_model)
        self.projection = nn.Linear(numwidows*6670, c_out, bias=True)
...
def Informerencoder():
    ...
    model = Informer(c_out=2, factor=5,d_model=numwidows,n_heads=head,e_layers=3, ffn_num_hiddens=numwidows*2, norm_shape=[6670,numwidows],ffn_num_input=numwidows,dropout=0.2,output_attention=True)
...
print(model)
input_matrix = torch.ones((1,6670,22))
# import thop
